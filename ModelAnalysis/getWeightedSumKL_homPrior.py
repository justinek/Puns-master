...
print "uniqueID,sentenceType,sumKL,sumSupport,sumBalance,sumDominance,numContextCombos"
...
for k, v in sentenceDict.iteritems():
        ...
        if len(subset1) > 0:
            ...
            for subset2 in contextSubsets:
                ...
                if len(subset2) > 0:
                    ...
                    # computes probability of context
                    # probability of a word being in the context (can tweak)
                    probWordInContext = 0.8
                    probC1 = math.pow(probWordInContext, len(c1))
                    probC2 = math.pow(probWordInContext, len(c2))
                    
                    # computes ratio between support and balance
                    support = (hom1c1Prob - hom2c1Prob) + (hom2c2Prob - hom1c2Prob)
                    balance = 1/(math.fabs(hom1c1Prob - hom2c2Prob) + 0.00001)
                    dominance = math.fabs(hom1c1Prob - hom2c2Prob)
                    ...
                    # computes KL normalized by probability of contexts
                    normalizedKL = KL * probC1 * probC2
                    normalizedSupport = support * probC1 * probC2
                    normalizedBalance = balance * probC1 * probC2
                    normalizedDominance = dominance * probC1 * probC2

                    sumKL = sumKL + normalizedKL
                    sumSupport = sumSupport + normalizedSupport
                    sumBalance = sumBalance + normalizedBalance
                    sumDominance = sumDominance + normalizedDominance

                    numContextCombos = numContextCombos + 1

                    # updates maxKL if current KL is greater than current maxKL
                    #if normalizedSupport > maxKL:
                    #    maxKL = normalizedSupport
    ...
    print uniqueID + "," + sentenceType + "," + str(sumKL) + "," + str(sumSupport) + "," + str(sumBalance) + "," + str(sumDominance) + "," + str(numContextCombos)
    #print ';'.join(maxC1),
    #print ",",
    #print ';'.join(maxC2),
