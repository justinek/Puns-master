...
r <- read.csv("../data_long_corrected.csv")
...
pairs <- read.csv("../../../Materials/wordPairs_orig.csv")
...
pairs.summary <- summarySE(pairs, measurevar="rating", groupvars=c("sentenceType", "wordPairVersion"))

# plot
ggplot(pairs.summary, aes(x=sentenceType, y=rating, fill=wordPairVersion)) +
...
# prints and saves data frame in a file
write.csv(pairs, "../wordPairs_orig_withRatings.csv")